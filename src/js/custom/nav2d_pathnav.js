...
function startNavigation(){
    ...
    timerInterval = document.getElementById('timeout').value;
    if(goalIter < goals.length ){
        goals[goalIter].send();
        goalIter++;
	if(goalIter == goals.length){ goalIter = 0;}
    }
    timer = setInterval(startNavigationLoop, 1000*timerInterval);
}


function startNavigationLoop(){
    console.log('Loop function'+ goalIter);
    if(goalIter < goals.length ){
        goals[goalIter].send();
        goalIter++;
	if(goalIter == goals.length){ goalIter = 0;}
